...
def faceDet2(data, dataDepth):
    nameDir = nameD
    namedepthDir = nameDepthDir
    global trainImages
    ...
    if len(faces) == 1:
        ...
        if trainCount == trainRate:
            ...
            #uncomment for RGB facerec #trainImages.append(roi_gray)
            trainImages.append(roi_depth)
...
def faceDet():
    ...
    depthdatasetDir = os.getcwd()+'/datasetdepth'
    ...
    nameDepthDir = depthdatasetDir + '/' + name

    #trainImages = []

    if not os.path.exists(nameDir):
        os.makedirs(nameDir)
    if not os.path.exists(nameDepthDir):
    	os.makedirs(nameDepthDir)

    global nameD
    nameD = datasetDir + '/' + name
    global datasetD
    datasetD = os.getcwd()+'/dataset'
    global datasetdepthD
    datasetdepthD = os.getcwd()+'/datasetdepth'
    print nameD
...
def createPKL(image_size):
    [images, labels, subject_names] = read_images(os.getcwd()+'/datasetdepth', image_size)
